public class ArrayBasedBinaryTreeNode<E extends Comparable<E>> implements BinaryTreeNode<E> {
    ...
    public String getStringForNode(int level){
        String returnString = "";
        
        if(hasLeftChild()){
            returnString+= ((ArrayBasedBinaryTreeNode<E>) getLeftChild()).getStringForNode(level+1);
        }

        for(int i=0; i<level;i++)
            returnString+= "    ";
        returnString+= getElement() + "\n";

        if(hasRightChild()){
            returnString+= ((ArrayBasedBinaryTreeNode<E>) getRightChild()).getStringForNode(level+1);
        }
